...
- The map opens centered on Barcelona by default. Self-hosted instances can change the starting location by setting `DEFAULT_MAP_LAT`, `DEFAULT_MAP_LNG`, and `DEFAULT_MAP_ZOOM` in their environment (or `docker-compose.yml`) so the map opens on their local area. Invalid or out-of-range values fall back to the built-in default.
