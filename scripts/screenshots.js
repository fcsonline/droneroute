...
(async () => {
  ...
  console.log("0/8 Main map...");
  {
    ...
    await page.screenshot({
      path: path.join(OUT, "main-map.jpg"),
      type: "jpeg",
      quality: 90,
    });
  ...
  console.log("1/8 Orbit template...");
  {
    ...
    await page.screenshot({
      path: path.join(OUT, "template-orbit.jpg"),
      type: "jpeg",
      quality: 85,
  ...
  console.log("2/8 Grid survey template...");
  {
    ...
    await page.screenshot({
      path: path.join(OUT, "template-grid.jpg"),
      type: "jpeg",
      quality: 85,
      clip: { x: box.x, y: box.y, width: box.width, height: box.height },
    });
  ...
  console.log("3/8 Facade scan template...");
  {
    ...
    await page.screenshot({
      path: path.join(OUT, "template-facade.jpg"),
      type: "jpeg",
      quality: 85,
      clip: { x: box.x, y: box.y, width: box.width, height: box.height },
    });
  ...
  console.log("4/8 Pencil path template...");
  {
    ...
    // Config panel should be visible with preview — crop to map area only
    await page.screenshot({
      path: path.join(OUT, "template-pencil.jpg"),
      type: "jpeg",
      quality: 85,
      clip: { x: box.x, y: box.y, width: box.width, height: box.height },
    });
  ...
  console.log("5/8 Building mission with waypoints + POI...");
  {
    ...
    const wpItems = page.locator(
      '[class*="flex items-center gap-2 rounded-md px-2"]',
    );
    ...
    await page.screenshot({
      path: path.join(OUT, "multiselect.jpg"),
      type: "jpeg",
      quality: 85,
    });
    ...
    if (await sidebar.isVisible()) {
      ...
      await page.screenshot({
        path: path.join(OUT, "elevation-graph.jpg"),
        type: "jpeg",
        quality: 85,
        clip: {
          x: sidebarBox.x,
