...
async function queryLayer(
  ...
  return json.features.map((f): AirspaceZone => {
    ...
    return {
      ...
      geometry: f.geometry,
      altitudeLower: layer.lowerField
        ? numOrUndef(p[layer.lowerField])
        : undefined,
      altitudeUpper: layer.upperField
        ? numOrUndef(p[layer.upperField])
        : undefined,
      description: layer.descriptionField
        ? String(p[layer.descriptionField] ?? "")
        : undefined,
