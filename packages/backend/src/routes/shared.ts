...
sharedRoutes.post("/missions/:id/share", authMiddleware, (req: AuthRequest, res) => {
  const db = getDb();
  const mission = db.prepare("SELECT id, user_id, share_token FROM missions WHERE id = ?").get(req.params.id) as any;
  ...
  db.prepare("UPDATE missions SET share_token = ? WHERE id = ?").run(shareToken, req.params.id);
...
sharedRoutes.delete("/missions/:id/share", authMiddleware, (req: AuthRequest, res) => {
  const db = getDb();
  const mission = db.prepare("SELECT id, user_id FROM missions WHERE id = ?").get(req.params.id) as any;
  ...
  db.prepare("UPDATE missions SET share_token = NULL WHERE id = ?").run(req.params.id);
